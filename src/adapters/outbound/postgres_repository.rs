...
impl PostgresRepository {
    ...
    async fn save_token_transfer(&self, transfer: &TokenTransfer) -> AppResult<()> {
        sqlx::query(
            r#"
            INSERT INTO token_transfers (signature, slot, from_address, to_address, amount, mint)
            VALUES ($1, $2, $3, $4, $5, $6)
    ...
    async fn save_raydium_swap(&self, swap: &RaydiumSwapEvent) -> AppResult<()> {
        sqlx::query(
            ...
            (signature, slot, amm_pool, signer, amount_in, min_amount_out, amount_received, mint_source, mint_destination)
    ...
    async fn save_jupiter_swap(&self, swap: &JupiterSwapEvent) -> AppResult<()> {
        sqlx::query(
            ...
            (signature, slot, signer, amm_pool, mint_in, mint_out, amount_in, amount_out, slippage_bps)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
            ON CONFLICT (signature) DO NOTHING
            "#,
        )
